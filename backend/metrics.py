"""Advanced valuation and quality metrics for stock screening / analysis."""
...
def calculate_undervaluation_score(stock_data):
    ...
    try:
        if stock_data["P/E Ratio"] < 20:
            scoring += 20
        peg = calculate_peg_ratio(stock_data)
        if peg is not None and peg < 1:
            scoring += 25
        graham = calculate_graham_number(stock_data)
        if stock_data["price"] is not None and graham is not None and stock_data["price"] < graham:
            scoring += 20
        dte = calculate_debt_to_equity(stock_data)
        if dte is not None and dte < 1:
            scoring += 15
        fcf_yield = calculate_free_cash_flow_yield(stock_data)
...
if __name__ == "__main__":
    from backend.stock_data import get_stock

    ticker = "AAPL"
    example_stock = get_stock(ticker)
